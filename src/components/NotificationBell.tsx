...
export function NotificationBell() {
  ...
  const load = useCallback(async () => {
    ...
    if (itemsResult.error || countResult.error) {
      setError(itemsResult.error?.message ?? countResult.error?.message ?? 'No pudimos cargar las notificaciones.');
    } else {
  ...
  const markAllRead = async () => {
    ...
    setMarkingAll(true);
    setError('');
    const { error: rpcError } = await client.rpc('mark_all_notifications_read');
    setMarkingAll(false);
    if (rpcError) {
  ...
  return (
    <div className="notification-bell-v112">
      <button
        type="button"
        className="notification-bell-trigger-v112"
      ...
      {open && (
        <section className="notification-popover-v112 notification-popover-v113">
          <header>
            <div><strong>Notificaciones</strong><span>{unreadCount} sin leer</span></div>
            {unreadCount > 0 && (
              <button type="button" onClick={() => { void markAllRead(); }} disabled={markingAll}>
                <CheckCheck size={14}/>{markingAll ? 'Marcando…' : 'Marcar todas'}
              </button>
            )}
          </header>

          {error && <p className="notification-error-v113">{error}</p>}
          {loading && items.length === 0 ? (
            <p className="notification-empty-v112">Cargando…</p>
          ) : items.length === 0 ? (
            <p className="notification-empty-v112">Aún no tienes notificaciones.</p>
          ) : (
            <div className="notification-list-v112">
              {items.map((item) => (
                <Link
                  key={item.id}
          ...
          <footer className="notification-footer-v113">
            <Link to="/notifications" onClick={() => setOpen(false)}>Ver todas las notificaciones</Link>
          </footer>
        </section>
      )}
    </div>
