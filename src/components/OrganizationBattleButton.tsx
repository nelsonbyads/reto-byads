...
interface GymRow { id: string; name: string; organization_type: string; country_code: string | null; verification_status: string; }
...
export function OrganizationBattleButton({ exercise, reps, diceLevel }: Props) {
  ...
  useEffect(() => {
    if (!open || !canManage || !verified || !user || !supabase || !activeWorkspace.organizationId) return;
    ...
    const load = async () => {
      ...
      const [membershipsResult, rivalsResult] = await Promise.all([
        ...
        client.from('organizations').select('id, name, organization_type, country_code, verification_status').eq('organization_type', 'gym').eq('verification_status', 'verified').order('name'),
      ]);
      if (!active) return;
    ...
    void load();
    return () => { active = false; };
  }, [activeWorkspace.organizationId, canManage, open, user, verified]);

  const close = () => { if (!sending) { setOpen(false); setError(''); setSentBattleId(null); } };
  const createBattle = async () => {
    if (!supabase || !activeWorkspace.organizationId || !rivalId || !canManage || !verified) return;
  ...
  if (!verified) return <button className="gym-battle-trigger-v121" type="button" disabled title="DadoFit debe verificar este Gym antes de crear batallas"><Trophy size={18}/> Gym vs Gym · requiere verificación</button>;
