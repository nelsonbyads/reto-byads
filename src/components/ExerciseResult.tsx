...
export function ExerciseResult({ exercise, reps, rollId, diceLevel, onDone }: Props) {
  ...
  return (
    <section className="result-card result-card-v7" aria-live="polite">
      <div className="result-header-v7"><div><span className="eyebrow">RESULTADO</span><h2>{exercise.name}</h2></div><span className="result-roll-badge-v7">{reps} REP</span></div>

      <div className="result-dashboard-v7">
        <div className="result-visual-v7"><div className="result-reps-v7"><strong>{reps}</strong><span>REPETICIONES</span></div><ExerciseMedia exercise={exercise}/></div>

        <div className="result-details-v7">
          <dl className="metadata-grid metadata-grid-v7"><div><dt>Equipo</dt><dd>{equipmentLabel(exercise.equipment)}</dd></div><div><dt>Zona</dt><dd>{bodyPartLabel(exercise.bodyPart)}</dd></div><div><dt>Objetivo</dt><dd>{exercise.target}</dd></div><div><dt>Grupo muscular</dt><dd>{exercise.muscleGroup || '—'}</dd></div></dl>
