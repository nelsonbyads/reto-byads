...
interface ParticipantRow { id: string; challenge_id: string; user_id: string; status: string; submitted_at: string | null; completed_at: string | null; reward_coins_granted: number; reward_xp_granted: number; sponsor_reward_points_granted: number; reward_block_reason: string | null; }
...
export function SponsoredChallengesPage() {
  ...
  const load = useCallback(async () => {
    ...
    const [campaignResult, orgResult, participantResult] = await Promise.all([
      ...
      challengeIds.length ? client.from('challenge_participants').select('id, challenge_id, user_id, status, submitted_at, completed_at, reward_coins_granted, reward_xp_granted, sponsor_reward_points_granted, reward_block_reason').eq('user_id', user.id).in('challenge_id', challengeIds) : Promise.resolve({ data: [] as ParticipantRow[], error: null }),
    ]);
