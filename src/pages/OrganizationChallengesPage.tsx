...
function rewardBlockLabel(reason: string | null, organizationName: string) {
  if (reason === 'organization_pair_cooldown') return `${organizationName} ya te otorgó una recompensa durante las últimas 24 horas.`;
  if (reason === 'organization_daily_limit') return 'Alcanzaste el límite de 5 recompensas de organizaciones durante las últimas 24 horas.';
  return 'Completado sin recompensa adicional por política anti-farming.';
}
...
function EvidencePanel({ participant, mode, onChanged }: { participant: ParticipantRow; mode: 'participant' | 'reviewer'; onChanged: () => Promise<void> }) {
  ...
  const review = async (decision: 'approved' | 'rejected') => {
    ...
    const coins = Number(result?.coins_granted ?? 0);
    const xp = Number(result?.xp_granted ?? 0);
    const gp = Number(result?.sponsor_points ?? 0);
    ...
    else if (coins + xp + gp === 0) setMessage('Reto aprobado. Este reto no tenía recompensa configurada.');
    else setMessage(`Aprobado: +${coins} DC · +${xp} XP · +${gp} GP.`);
    await onChanged();
  };
        ...
        {!cloudReady ? <section className="profile-card-v9 profile-cloud-callout-v9"><h2>Necesitas una cuenta cloud</h2></section> : <>
          ...
          {loading ? <section className="profile-card-v9">Cargando retos…</section> : tab === 'mine' ? (
            <section className="organization-challenge-list-v12">{ownParticipants.length === 0 ? <div className="profile-card-v9 organization-empty-v12"><Building2 size={30}/><h3>No tienes retos de organización</h3><p>Cuando un Gym u organización te publique uno, aparecerá aquí.</p></div> : ownParticipants.map((participant) => {
              ...
              return <article key={participant.id} className="profile-card-v9 organization-challenge-card-v12"><header><div><span className="eyebrow">{org?.name || 'ORGANIZACIÓN'}</span><h2>{challenge.reps}× {challenge.exercise_name}</h2><p>{challenge.dice_level.toUpperCase()} · hasta {formatDate(challenge.expires_at)}</p></div><span className={`organization-status-v12 status-${participant.status}`}>{statusLabel(participant.status)}</span></header><div className={`organization-reward-state-v153 ${participant.status === 'approved' ? 'final' : 'potential'}`}><strong>{participant.status === 'approved' ? 'Recompensa otorgada' : 'Recompensa potencial'}</strong><span>{participant.status === 'approved' ? 'Resultado real de esta aprobación' : 'Sujeta a aprobación y reglas anti-farming'}</span></div><div className="organization-challenge-rewards-v12"><span><Coins size={14}/>{participant.status === 'approved' ? participant.reward_coins_granted : challenge.reward_coins} DC</span><span><Trophy size={14}/>{participant.status === 'approved' ? participant.reward_xp_granted : challenge.reward_xp} XP</span><span><Building2 size={14}/>{participant.status === 'approved' ? participant.sponsor_points_granted : challenge.sponsor_points} GP</span></div>{participant.status === 'invited' && <div className="organization-response-v12"><button className="reject" onClick={() => { void respond(participant.id, false); }} disabled={Boolean(acting)}><X size={15}/> No participar</button><button onClick={() => { void respond(participant.id, true); }} disabled={Boolean(acting)}><Check size={15}/> Aceptar reto</button></div>}{participant.status === 'approved' && <div className={`organization-approved-v12 ${participant.reward_block_reason ? 'reward-blocked-v153' : ''}`}><Check size={18}/><div><strong>{participant.reward_block_reason ? 'Reto aprobado · sin recompensa' : 'Reto aprobado · recompensa aplicada'}</strong><span>{participant.reward_block_reason ? rewardBlockLabel(participant.reward_block_reason, org?.name || 'Esta organización') : `+${participant.reward_coins_granted} DC · +${participant.reward_xp_granted} XP · +${participant.sponsor_points_granted} GP`}</span></div></div>}{['accepted', 'rejected', 'submitted', 'approved'].includes(participant.status) && <EvidencePanel participant={participant} mode="participant" onChanged={load}/>}</article>;
            })}</section>
          ) : tab === 'review' ? (
