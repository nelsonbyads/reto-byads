-- DadoFit V13.2 - Sponsored Challenge Goal Types
-- Adds typed challenge objectives: repetitions, time, distance and quantity.

drop function if exists public.publish_sponsored_challenge(uuid, text, integer, integer, integer, text);
...
begin
  ...
  if (p_goal_type = 'repetitions' and p_goal_unit <> 'reps')
     ...
     or (p_goal_type = 'quantity' and p_goal_unit not in ('steps', 'times', 'units')) then
    raise exception 'invalid goal unit';
  end if;
  ...
  v_legacy_reps := case
    when p_goal_type = 'repetitions' then greatest(1, least(1000, round(p_goal_value)::integer))
    else 1
  end;
  ...
  insert into public.challenges (
    ...
    reward_coins, reward_xp, team_points, sponsor_points, sponsor_campaign_id, max_participants, metadata
  ) values (
    'organization', v_campaign.organization_id, 'sponsored', 'active', left(v_campaign.name, 160), v_campaign.description,
    v_exercise_id, left(btrim(p_exercise_name), 160), v_legacy_reps, 'amateur', now(),
    now() + make_interval(hours => p_duration_hours), true, least(v_campaign.default_reward_coins, 50),
    least(v_campaign.default_reward_xp, 100), 0, 0, v_campaign.id, p_max_participants,
    jsonb_build_object(
...
as $$
  select jsonb_build_object(
    'ok', true,
    'schema_version', 'v13.2-sponsored-goal-types',
